# Perfect XOR

We're given a file [decrypt.py](/hacktivitycon/perfect_xor/decrypt.py)

```Python
import base64
n = 1
i = 0
cipher_b64 = b""

def a(n):
    b = 0
    for i in range(1, n):
        if(n % i == 0):
            b += i
    return b == n

print("flag{", end='', flush=True)
cipher = base64.b64decode(cipher_b64).decode().split(",")
while(i < len(cipher)):
    if (a(n)):
        print(chr(int(cipher[i]) ^ n), end='', flush=True)
        i += 1
    n+=1

print("}")
```
let's decode base64 the cipher

`c=['114', '84', '437', '8095', '33550434', '8589869170', '137438691376', '2305843008139952235', '2658455991569831744654692615953842245', '191561942608236107294793378084303638130997321548169294', '13164036458569648337239753460458722910223472318386943117783728223', '14474011154664524427946373126085988481573677491474835889066354349131199152216', '', ''] `
we notice that this function
```Python
def a(n):
    b = 0
    for i in range(1, n):
        if(n % i == 0):
            b += i
    return b == n
```
computes the sum of divisors of `n` and checks if the sum equals n or not. If sum of idivors equals n, then n is xored with cipher. Those numbers are called **perfect numbers**
Since the length of our table is 14, we can easily get the 15 first from OEIS https://oeis.org/A000396/b000396.txt

No we have our perfect numbers in a list called `num`

We can then get the flag with this script
```Python
''.join(chr(i^j) for i,j in zip(cipher, num))
```

